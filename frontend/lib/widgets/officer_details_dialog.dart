...
class OfficerDetailsDialog extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final uid = officerData['registration_uid']?.toString() ?? officerData['uid']?.toString() ?? '';
    final name = officerData['name']?.toString() ?? officerData['title']?.toString() ?? 'Officer';
    final desig = officerData['designation']?.toString() ?? '';
    final email = officerData['email']?.toString() ?? '';
    final phone = officerData['phone']?.toString() ?? '';
    final badge = officerData['badge_number']?.toString() ?? officerData['govt_id']?.toString() ?? '';
    final roleId = officerData['role_id']?.toString() ?? '';
    final station = officerData['target_station']?.toString() ?? officerData['station_name']?.toString() ?? '';
    final district = officerData['target_district']?.toString() ?? officerData['district']?.toString() ?? '';
    ...
    final isStateLevel = roleId.contains('state') || roleId.contains('dgp') || roleId.contains('master') || desig.toLowerCase().contains('ig') || desig.toLowerCase().contains('cp');
    final isDivLevel = roleId.contains('div') || desig.toLowerCase().contains('dig') || desig.toLowerCase().contains('sp');
              ...
              child: SingleChildScrollView(
                ...
                child: Column(
                  ...
                  children: [
                      ...
                      child: Row(
                        children: [
                          CircleAvatar(
                            ...
                            child: photoUrl.isEmpty ? const Icon(Icons.person_rounded, size: 28, color: AppColors.navyDark) : null,
                          ),
                          const SizedBox(width: 12),
                          Expanded(
                            child: Column(
                              ...
                              children: [
                                Text(
                                  name.isNotEmpty ? name : 'Officer Registration Request',
                                  style: GoogleFonts.poppins(fontSize: 14, fontWeight: FontWeight.w700, color: AppColors.navyDark),
                                ),
                                if (desig.isNotEmpty)
                                  Text(
                                    desig,
                                    style: GoogleFonts.poppins(fontSize: 11.5, fontWeight: FontWeight.w600, color: Colors.blue.shade800),
                                  ),
                                ...
                                Container(
                                  ...
                                  child: Text(
                                    ...
                                    style: GoogleFonts.poppins(fontSize: 9.5, fontWeight: FontWeight.w800, color: Colors.orange.shade900),
                                  ),
                                ),
                              ],
                            ),
                    ...
                    _detailRow(
                      'Police Division',
                      divName.isNotEmpty ? divName : (isStateLevel ? 'Not Applicable (State Admin)' : ''),
                    ),
                    _detailRow(
                      'District Range',
                      district.isNotEmpty ? district : (isStateLevel ? 'Not Applicable (State Admin)' : ''),
                    ),
                    _detailRow(
                      'Police Station',
                      station.isNotEmpty ? station : ((isStateLevel || isDivLevel) ? 'Not Applicable (High Rank / Command)' : ''),
                    ),
                      ...
                      Expanded(
                        child: SizedBox(
                          ...
                          child: OutlinedButton.icon(
                            ...
                            label: Text('Reject Request', style: GoogleFonts.poppins(fontSize: 12, fontWeight: FontWeight.w600)),
                      ...
                      Expanded(
                        child: SizedBox(
                          ...
                          child: FilledButton.icon(
                            ...
                            label: Text('Approve Officer', style: GoogleFonts.poppins(fontSize: 12, fontWeight: FontWeight.w600)),
          ...
          Expanded(
            child: isNotApplicable
                ? Container(
                    ...
                    child: Text(
                      'Not Applicable',
                      style: GoogleFonts.poppins(fontSize: 10.5, fontWeight: FontWeight.w700, color: Colors.blue.shade800),
                    ),
                  )
                : isEmpty
                    ? Container(
                        ...
                        child: Text(
                          'Not Submitted',
                          style: GoogleFonts.poppins(fontSize: 10.5, fontWeight: FontWeight.w600, color: Colors.grey.shade600, fontStyle: FontStyle.italic),
                        ),
                      )
                    : Text(
                        value,
                        style: GoogleFonts.poppins(fontSize: 12, fontWeight: FontWeight.w600, color: AppColors.navyDark),
                      ),
          ),
        ],
      ),
  ...
  Widget _documentPreview({required String label, required String url, required IconData icon}) {
        ...
        Container(
          ...
          child: hasUrl
              ? Image.network(
                  ...
                  errorBuilder: (_, __, ___) => _noDocWidget('Image Load Error'),
                )
  ...
  Widget _noDocWidget(String text) {
    return Center(
      child: Column(
        ...
        children: [
          Icon(Icons.insert_drive_file_outlined, size: 20, color: Colors.grey.shade400),
          const SizedBox(height: 2),
          Text(
            text,
            style: GoogleFonts.poppins(fontSize: 10, color: Colors.grey.shade500, fontStyle: FontStyle.italic),
          ),
        ],
      ),
    );
