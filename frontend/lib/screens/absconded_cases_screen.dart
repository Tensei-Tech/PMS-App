// lib/screens/absconded_cases_screen.dart
// Absconded cases screen matching Form I-V style UI:
// Status tabs (Total / Pending / Disposal), Date Calendar picker & sort,
// Notepad-style case cards, category filters, and quick New Case FAB.
...
class AbscondedCasesScreen extends StatefulWidget {
  final bool readOnly;

  const AbscondedCasesScreen({super.key, this.readOnly = false});

  @override
  State<AbscondedCasesScreen> createState() => _AbscondedCasesScreenState();
}

class _AbscondedCasesScreenState extends State<AbscondedCasesScreen> {
  ...
  String _recordCategory(ModuleRecord r) {
    final sub = r.subCategory?.trim() ?? '';
    if (sub.isNotEmpty && sub.toLowerCase() != 'absconded') {
      return sub;
    }
    if (r.title.contains('—')) {
      final prefix = r.title.split('—').first.trim();
      if (prefix.isNotEmpty && prefix.toLowerCase() != 'absconded') {
        return prefix;
      }
    }
    return '';
  ...
  bool _recordMatchesCategory(ModuleRecord r) {
    if (_selectedCategory == null || _selectedCategory == 'All') return true;
    final cat = _recordCategory(r);
  ...
  List<ModuleRecord> _filteredAndSortedRecords(List<ModuleRecord> records) {
    final filtered = records
        .where(_recordMatchesCategory)
        .where(_recordMatchesDate)
        .toList();
  ...
  Widget _buildCategoryFilterChips(List<ModuleRecord> records) {
    // Collect distinct crime categories from records
    ...
    if (catCounts.isEmpty) return const SizedBox.shrink();
    ...
    return Container(
      ...
      child: SingleChildScrollView(
        ...
        child: Row(
          children: [
            Text(
              TranslationHelper.translate(context, 'Crime Head:'),
              style: GoogleFonts.poppins(
                fontSize: 11.5,
                fontWeight: FontWeight.w600,
                color: AppColors.lightSubText,
  ...
  Widget _buildCatChip({
    ...
    return InkWell(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          color: isSelected ? AppColors.navyMid : const Color(0xFFF1F5F9),
          borderRadius: BorderRadius.circular(16),
          border: Border.all(
            color: isSelected ? AppColors.navyMid : const Color(0xFFCBD5E1),
          ),
        ),
        child: Row(
          mainAxisSize: MainAxisSize.min,
            ...
            Container(
              ...
              child: Text(
                ...
                style: GoogleFonts.poppins(
                  fontSize: 10,
                  fontWeight: FontWeight.w700,
                  color: isSelected ? Colors.white : AppColors.lightSubText,
                ),
              ),
            ),
          ],
  ...
  Widget build(BuildContext context) {
    final provider = context.watch<AbscondedProvider>();
    ...
    return Scaffold(
      ...
      body: Column(
        ...
        children: [
          ...
          Container(
            ...
            child: Row(
              children: [
                ...
                const SizedBox(width: 8),
                Text(
                  TranslationHelper.translate(context, 'Absconded Cases'),
                  style: GoogleFonts.poppins(
                    fontSize: 13,
                    fontWeight: FontWeight.w700,
                    color: AppColors.navyDark,
          ...
          _buildCategoryFilterChips(statusRecords),
          ...
          Expanded(
            child: visibleRecords.isEmpty
                ...
                : ListView.builder(
                    key: PageStorageKey(
                      'absconded_${_selectedStatusTab.name}_${_selectedCategory ?? "all"}',
                    ),
