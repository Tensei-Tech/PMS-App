// lib/screens/pending_io_wise_screens.dart
// Pending Cases → category → IO Wise grouping (live consolidated records).
...
/// Same consolidated module list shape as ModuleHubScreen pending aggregation.
...
class PendingIoWiseByCategoryScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final filtered = consolidated
        .where((r) => pendingIoWiseEligibleInCategory(r, category))
        .toList();
    ...
    final title = 'IO Wise Pending - $category';
                              ...
                              onTap: () {
                                Navigator.push(
                                  ...
                                  AppTheme.fadeSlideRoute(
                                    page: PendingIoWiseDetailScreen(
                                      category: category,
                                      ioDisplayName: io,
                                    ),
                                  ),
...
class PendingIoWiseDetailScreen extends StatelessWidget {
  final String category;
  final String ioDisplayName;

  const PendingIoWiseDetailScreen({
    super.key,
    required this.category,
  ...
  Widget build(BuildContext context) {
    ...
    final mine = consolidated
        .where(
          (r) =>
              pendingIoWiseEligibleInCategory(r, category) &&
