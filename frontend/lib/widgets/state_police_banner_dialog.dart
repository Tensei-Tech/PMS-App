...
class StatePoliceBannerDialog extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final designation = auth.designation.isNotEmpty ? auth.designation : (user?.designation ?? '');
    final district = auth.district.isNotEmpty ? auth.district : (user?.district ?? 'Headquarters');
    final station = auth.homeStationName.isNotEmpty ? auth.homeStationName : (user?.stationName ?? 'State HQ');
          ...
          Container(
            ...
            child: Row(
              children: [
                ...
                Expanded(
                  child: Text(
                    'Serving ${branding.stateName} State with Honor, Integrity, and Excellence. Integrated via Khakhi Diary Enterprise Portal.',
                    style: GoogleFonts.poppins(
                      fontSize: 11.5,
